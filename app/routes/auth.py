...
SECRET_KEY = "supersecretkey"  # bro isko env file me dalna
...
def login():
    ...
    if user and check_password_hash(user.password, password):
        ...
        resp.set_cookie(
            ...
            samesite="Lax",       # React + Flask ke liye safe
...
def check_login():
    ...
    if not token:
        return jsonify({"success": False, "message": "No token found"}), 401

    try:
        data = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
        return jsonify({"success": True, "user": data["username"]}), 200
